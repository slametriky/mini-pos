...
@section('content')
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="modalUploadFileList">
        <div class="modal-dialog modal-lg role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">Upload Data Pendukung</h5>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
                <button class="btn btn-primary" onclick="tambahFile()">Upload File</button>
        ...
        <div class="modal-dialog modal-lg role="document">
          <div class="modal-content">
            ...
            <form id="formUpload" method="post">                
                <div class="modal-body">                                                             
                    <div class="form-group row">
                        <label class="col-form-label col-12 col-md-2 col-lg-2">Jenis Data</label>                       
                        <div class="col-sm-12 col-md-6">
                            <input type="hidden" id="data_id" name="data_id">
                            <select name="jenis_file" class="form-control" required>
                                <option value="">Pilih Jenis Data</option>
                                <option value="1">KAK (Kerangka Acuan Kerja)</option>
                                <option value="2">RAB (Rincian Anggaran Biaya)</option>
                                <option value="3">Data pendukung (Seperti brosur atau surat tawaran dari perusahaan, atau pricelist atau gambar kerja serta perhitungan konsultan)</option>
                            </select>
                        </div>    
                    </div>                                                           
                    <div class="form-group row">
    ...
    <script>                
                                    ...
                                    ${(() => {
                                        ...
                                        if(data.status == 0){
                                            if (data.level == 'program') {
                                                ...
                                                <button class="btn btn-primary btn-custom" title="Data Pendukung"  onclick="uploadFile(${data.id}, 'subKom')"><i class="fa fa-file-pdf"></i></button>
                                                ...
                                                <button class="btn btn-primary btn-custom" title="Data Pendukung"  onclick="uploadFile(${data.id}, 'detail')"><i class="fa fa-file-pdf"></i></button>
                        ...
                        data_tr += `
                            <tr>
                                ...
                                <td>
                                    ${(() => {
                                        if(data.jenis_file == 1) {
                                            return `KAK`;
                                        } else if(data.jenis_file == 2) {
                                            return `RAB`   
                                        } else if(data.jenis_file == 3) {
                                ...
                                <td>
                                    ...
                                    <button class="btn btn-sm btn-danger" onclick="hapusFileDetail(${data.id})"><i class="fa fa-trash"></i></button>
                                </td>
                            </tr>
                        `;
                    })
                    ...
                    res.data.forEach((data, index) => {
                        data_tr += `
                            <tr>
                                ...
                                <td>
                                    <a class="btn btn-primary btn-sm" target="_blank" title="lihat" href="{{ asset('assets/file/${data.file}') }}" role="button"><i class="fa fa-eye"></i></a>
                                    <button class="btn btn-sm btn-danger" onclick="hapusFileSubKom(${data.id})"><i class="fa fa-trash"></i></button>
                                </td>
                            </tr>
                        `;
                    })
        ...
        cekAsset = () => {
            ...
            if(dataProgram[0].status ==  0){                
                dataTmpStatus = `
                    <button class="btn btn-success mb-2 float-right" onclick="verifikasi()">Verifikasi</button>
                `;
                ...
                dataTmpStatus = `
                    <div class="offset-md-4 col-md-4">
                        <h4 class="text-center text-success">Sudah Verifikasi</h4>
                    </div>
                    <div class="col-md-4">
                        <button class="btn btn-danger mb-2 float-right" onclick="batalVerifikasi()">Batal Verifikasi</button>
                    </div>
                `;
        ...
        verifikasi = () => {
            Swal.fire({
                ...
                if(result.value){
                    ...
                    axios.post('/unit/verifikasi_rka', {                                
                        'tahun':tmpTahun
                    })
                    .then((response) => {                            
                        if(response.data.responCode == 1){
